...
namespace btl_backend.Controllers
{
    ...
    public class FavoritesController : ControllerBase
    {
        ...
        public async Task<ActionResult> AddFavorite(int coffeeShopId, [FromBody] int? userId = null)
        {
            // Lấy userId từ body hoặc dùng mặc định
            var currentUserId = userId ?? 1; // TODO: Get from JWT token
            ...
            var existingFavorite = await _context.Favorites
                .FirstOrDefaultAsync(f => f.UserId == currentUserId && f.CoffeeShopId == coffeeShopId);
        ...
        public async Task<ActionResult> RemoveFavorite(int coffeeShopId, [FromBody] int? userId = null)
        {
            // Lấy userId từ body hoặc dùng mặc định
            var currentUserId = userId ?? 1; // TODO: Get from JWT token
        ...
        public async Task<ActionResult> IsFavorite(int coffeeShopId, [FromQuery] int? userId = null)
        {
            // Lấy userId từ query hoặc dùng mặc định
            var currentUserId = userId ?? 1; // TODO: Get from JWT token

            var isFavorite = await _context.Favorites
                .AnyAsync(f => f.UserId == currentUserId && f.CoffeeShopId == coffeeShopId);
